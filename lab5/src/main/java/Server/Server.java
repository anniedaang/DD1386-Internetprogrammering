...
public class Server {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            ...
            InputStream is = new FileInputStream(new File("/Users/anniedang/.keystore"));
            ks.load(is, "andaanda".toCharArray()); // loads the keystore file located at /Users/anniedang/.keystore and sets the password as "andaanda".

            // This section creates a KeyManagerFactory object to create KeyManager instances.
            KeyManagerFactory kmf = KeyManagerFactory.getInstance(KeyManagerFactory.getDefaultAlgorithm());
            kmf.init(ks, "andaanda".toCharArray()); // It initializes the key manager factory with the keystore and its password.
            KeyManager km[] = kmf.getKeyManagers(); // It retrieves an array of KeyManager instances from the factory.

            // Set up SSL context and parameters
            SSLContext sslContext = SSLContext.getInstance("TLS"); // sets up the SSL context using the TLS protocol.
            sslContext.init(km, null, null);
            SSLServerSocketFactory ssf = null;
            ssf= sslContext.getServerSocketFactory();
            SSLServerSocket ss = null;
            ss = (SSLServerSocket) ssf.createServerSocket(8989);

            while (true) {

                try (Socket socket = ss.accept(); // wait for a client to connect
